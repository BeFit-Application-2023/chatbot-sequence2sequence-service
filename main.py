...
from schemas import Seq2SeqTextSchema
from config import ConfigManager
from seq2seq import SequenceGenerator
from seq2seq.vocabulary import Voc
...
sequence2sequence_schema = Seq2SeqTextSchema()
...
sequence2sequence_model = SequenceGenerator(
    config.neural_network.encoder_path,
    config.neural_network.decoder_path,
    config.neural_network.vocabulary_path,
    config.encoder_config.__dict__,
    config.decoder_config.__dict__
)

# Creation of the Task Executor.
TASK_EXECUTOR = TaskExecutorManager(config.neural_network, sequence2sequence_model)
...
class SentimentModel(db.Model):
    ...
    __tablename__ = 'sequence2sequence'
    ...
    prediction = db.Column(db.Text, unique=False)
...
def intent():
    ...
    if check_response != "OK":
        ...
        result, status_code = sequence2sequence_schema.validate_json(request.json)
        if status_code != 200:
